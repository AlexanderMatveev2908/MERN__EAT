export const quickSortDate = (arr, val, cbProcessOrder) => {
    if (arr.length <= 1)
        return arr;
    ...
    for (const order of arr) {
        ...
        if (val === 1) {
            if (dateOrder < datePivot)
                left.push(order);
            else if (dateOrder > datePivot)
                right.push(order);
            else
                equal.push(order);
        }
...
const innerMergePrice = (left, right, val) => {
    const res = [];
    let i = 0, j = 0;
    while (i < left.length && j < right.length) {
        const leftVal = left[i].totPrice + left[i].delivery - left[i].discount;
        const rightVal = right[j].totPrice + right[j].delivery - right[j].discount;
...
export const mergeSortPrice = (arr, val) => {
    if (arr.length <= 1)
        return arr;
...
const heapifyAsc = (arr, n, i) => {
    ...
    if (left < n && arr[left].discount > arr[bigger].discount)
        bigger = left;
    if (right < n && arr[right].discount > arr[bigger].discount)
        bigger = right;
...
export const heapDiscountAsc = (arr) => {
    const n = arr.length;
...
const heapifyDesc = (arr, n, i) => {
    ...
    // and so on...
    if (left < n && arr[left].discount < arr[smaller].discount)
        smaller = left;
    if (right < n && arr[right].discount < arr[smaller].discount)
        smaller = right;
    if (smaller !== i) {
...
export const heapDiscountDesc = (arr) => {
    const n = arr.length;
    let i = Math.floor(n / 2) - 1;
    do {
        heapifyDesc(arr, n, i);
